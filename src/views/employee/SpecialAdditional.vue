  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button
            :disabled="scope.row.apply_state !== 0"
            size="mini"
            type="success"
            plain
...
<script>
import {agreeApply, deleteApply, downloadApplyFile, rejectApply, searchAddApply} from '@/api/addition'
import Qs from 'qs'
...
export default {
  ...
  methods: {
    ...
    handleAlter(row) {
      this.$router.push({ path: '/employee/additional/alterapply/index', name: 'AlterApply', query: { row: JSON.stringify(row) }})
    },
    handleDelete(row) {
